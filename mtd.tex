...
\title[Quantifying QGP properties through model-to-data comparison]
...
\begin{frame}[plain,noframenumbering]
  ...
  PRC accepted, arXiv:1502.00339 [nucl-th].
  ...
  \vskip1em
  \scriptsize
  CIPANP 2015\hskip2ex\textbar\hskip2ex Friday, May 22
\end{frame}
...
\begin{frame}{Collective flow and $\eta/s$}
  \begin{columns}
    ...
    \includegraphics[width=\textwidth]{third_party/evolution5}
    ...
    \begin{center}
      \begin{tikzpicture}
        \small
        \matrix[ampersand replacement=\&, column sep=2ex, row sep=2ex] {
          \coordinate (L) at (-.48, 0);
          \coordinate (R) at ( .48, 0);
          \begin{scope}
            \clip (L) circle (.8);
            \clip (R) circle (.8);
            \shade[inner color=yellow, outer color=red] (0, 0) ellipse (.4 and .7);
          \end{scope}
          \begin{scope}[thick]
            \draw (L) circle (.8);
            \draw (R) circle (.8);
          \end{scope}
          \draw[->] (L) -- (R) node[above] {$b$}; \&
          \node {0 fm/$c$}; \\
          \begin{scope}[semithick, ->]
            \shade[inner color=red, outer color=black] (0, 0) ellipse (.6 and .3);
            \draw (0:.7) -- (0:1.2);
            \draw (0:-.7) -- (0:-1.2);
            \draw (90:.4) -- (90:.6);
            \draw (-90:.4) -- (-90:.6);
    ...
    \end{center}
    \raggedright
    \parbox[t][.5\textwidth][t]{\textwidth}{
      \only<1>{%
        Initial spatial anisotropy \\
        $\rightarrow$ final momentum anisotropy (flow) \\[1ex]
        Shear viscosity $\eta/s$ washes out \\ collective behavior \\
        $\rightarrow$ suppresses flow
      }
      \only<2>{%
        Parametrize momentum anisotropy by flow (Fourier) coefficients $v_n$
        \begin{equation*}
          \frac{dN}{d\phi} \propto 1 + \sum_n v_n \cos [ n(\phi - \psi_n) ]
        \end{equation*}
        $v_2$ (elliptic), $v_3$ (triangular), \ldots \\[1ex]
        increase $\eta/s$ $\implies$ $v_n$ decrease
      }
    }
...
\begin{frame}{Gaussian process emulator}
  ...
  \begin{columns}[c]
    \column{.52\textwidth}
    \tikz\node[text width=\textwidth, align=justify, inner sep=1ex, draw, semithick, color=theme] {
...
\begin{frame}{Validation}
  \begin{itemize}
    \item Run model at each validation point
