...
\title
[Quantifying QCD properties through model-to-data comparison]
...
\begin{frame}{Calibration}
  ...
  \end{equation*}
  \emph{Assume} true parameters $\xs$ exist $\rightarrow$ find probability dist.\ for $\xs$ \\[1.5ex]
  Bayes' theorem:
  \begin{equation*}
...
\begin{frame}{Next}
  \begin{itemize}
    ...
    \item Parametric initial condition models
      \begin{itemize}
        \item Trento (see Scott Moreland's talk tomorrow)
        \item fluctuated Glauber
      \end{itemize}
    \item More input parameters: nucleon size, \\ temperature-dependent $\eta/s$, bulk viscosity, $T_\text{switch}$
