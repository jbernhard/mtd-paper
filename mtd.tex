...
\begin{enumerate}
  ...
  \item Subtract the mean of each column and transform $Y$ into principal components $Z_q$ with $q = 5$ PCs, retaining over 99\% of the variance as shown in Fig.~\ref{fig:pc_var}.
    For the exact principal components see table \ref{tab:pc} at the end of the appendix.
\end{enumerate}
...
\begin{table*}
  \caption{
    ...
    Each row contains the coefficients for the indicated observable;
    each column contains the coefficients for the indicated PC.
    The bottom row contains the PC weights (explained fraction of the total variance).
    A given PC is computed as a linear combination of the observables with coefficients in the corresponding column.
    Note that the PC transform was computed \emph{after} several preprocessing steps as described in section~\ref{sec:multiout}.
    The overall sign of each column is unimportant; only the relative sign and magnitude is meaningful.
    Essentially, the $18 \times 5$ blocks for Glauber and KLN are the matrices $U_q$ in Eq.~\eqref{eq:pcreddim} which transform the preprocessed data $Y$ into PCs $Z_q$.
  }
  \input{tables/principal_components}
\end{table*}
